...
public abstract class AbstractInformationDAO<T> {
    ...
    private void setDatabaseInformation() {
        ...
        jdbcDriver = Optional.ofNullable(System.getProperty("JDBC_DRIVER")).orElse(jdbcDriver);
        databaseURL = Optional.ofNullable(System.getProperty("DATABASE_URL")).orElse(databaseURL);
        userName = Optional.ofNullable(System.getProperty("USERNAME")).orElse(userName);
        passWord = Optional.ofNullable(System.getProperty("PASSWORD")).orElse(passWord);
